...
/** Adds a comment to a class stream post (any class member). */
export async function createStreamComment(input: {
  postId: string;
  classId: string;
  body: string;
}): Promise<StreamActionResult> {
  ...
  const supabase = await createSessionClient();
  const { data, error } = await supabase
    .from("class_stream_comments")
    .insert({ post_id: parsed.data.postId, body: parsed.data.body })
    .select("id")
    .single();
...
/** Deletes a comment (RLS: the author, the class teacher, or an admin). */
export async function deleteStreamComment(input: {
  id: string;
  classId: string;
}): Promise<StreamActionResult> {
  const t = await getT();
  ...
  const supabase = await createSessionClient();
  const { error } = await supabase.from("class_stream_comments").delete().eq("id", parsed.data.id);
  if (error) return { ok: false, error: t.stream.error };
